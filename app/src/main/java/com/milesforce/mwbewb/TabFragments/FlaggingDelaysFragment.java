...
import static com.milesforce.mwbewb.Utils.ConstantUtills.Delays_url;
 ...
 */
public class FlaggingDelaysFragment extends Fragment {

    RecyclerView delaysRecyclerView;
    DelaysAdapter delaysAdapter;
    ApiClient apiClient;
    ArrayList<DelaysModel> delaysModelArrayList;
    SwipeRefreshLayout delaysSwipetoRefresh;
    String AccessToken;
    int page = 1;
    ProgressBar delays_progress, bottom_delaysProgress;
    FloatingActionButton flating_btn, delays_refresh_floating;
    SharedPreferences sharedPreferences;



    public FlaggingDelaysFragment() {
        // Required empty public constructor
    }

    public static FlaggingDelaysFragment newInstance() {
        FlaggingDelaysFragment fragment = new FlaggingDelaysFragment();
        return fragment;
    }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_flagging_delays, container, false);
    }
    ...
    private void getDelaysdata() {
        delays_progress.setVisibility(View.VISIBLE);
        delaysModelArrayList = new ArrayList<>();
        page = 1;
        apiClient.getDelaysModel(Delays_url,1, "Bearer " + AccessToken, "application/json").enqueue(new Callback<ModelForAllData>() {
            @Override
                ...
                try {
                    ...
                    if (response.body() == null) {
                        ...
                        if (statusCode ==401) {
                            ...
                            delays_progress.setVisibility(View.GONE);
                        }
                    } else {
                        ...
                        delaysAdapter = new DelaysAdapter(getContext(), delaysModelArrayList);
                        delaysRecyclerView.setAdapter(delaysAdapter);
                        delaysAdapter.notifyDataSetChanged();
                        flating_btn.setImageBitmap(textAsBitmap(String.valueOf(response.body().getDashboard_data().getTotal()), 40, Color.BLUE));


                        delaysAdapter.onItemClickListners(new DelaysAdapter.OnClickListeners() {
                            @Override
    ...
    private void initViews(View view) {
        delaysSwipetoRefresh = view.findViewById(R.id.delaysSwipetoRefresh);
        delaysRecyclerView = view.findViewById(R.id.delaysRecyclerView);
        delays_progress = view.findViewById(R.id.delays_progress);
        flating_btn = view.findViewById(R.id.flating_btn);
        bottom_delaysProgress = view.findViewById(R.id.bottom_delaysProgress);
        delaysRecyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
        delaysSwipetoRefresh.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                delaysSwipetoRefresh.setRefreshing(false);
            }
        });
        paginatedRecyclerview(delaysRecyclerView);
        flating_btn.setImageBitmap(textAsBitmap(String.valueOf("0"), 40, Color.BLUE));
        delays_refresh_floating = view.findViewById(R.id.delays_refresh_floating);
        delays_refresh_floating.setOnClickListener(new View.OnClickListener() {
            @Override
    ...
    private void paginatedRecyclerview(RecyclerView delaysRecyclerView) {
        delaysRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
            @Override
            ...
            public void onScrolled(@NonNull RecyclerView recyclerView, int dx, int dy) {
                ...
                if (dx == 0 && dy == 0) {
                    ...
                    if (Objects.requireNonNull(linearLayoutManager).findLastCompletelyVisibleItemPosition() ==
                            delaysAdapter.getItemCount() - 1) {
                        page = page + 1;
                        bottom_delaysProgress.setVisibility(View.VISIBLE);
                        apiClient.getDelaysModel(Delays_url,page, "Bearer " + AccessToken, "application/json").enqueue(new Callback<ModelForAllData>() {
                            @Override
                            public void onResponse(Call<ModelForAllData> call, Response<ModelForAllData> response) {
                                try {
                                    ...
                                    delaysAdapter.notifyDataSetChanged();
                                    bottom_delaysProgress.setVisibility(View.GONE);
                                } catch (Exception e) {
                                    bottom_delaysProgress.setVisibility(View.GONE);
